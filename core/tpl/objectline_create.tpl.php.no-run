		...
		if (is_object($objectline)) {
			...
			if ($object->element == "facture") {
				...
				if (empty($conf->global->MAIN_DISABLE_FREE_LINES)) {
					...
					print '<label class="fieldrequired" style="padding-right:2rem;" for="umed">Unidad de Medida</label><input type="text" name="umed" id="umed">';
					print '</div>';
					print '<div style="padding-top: 10px" >';
					print '<label class="fieldrequired" style="padding-right:2rem;" for="claveprodserv">Clave Producto Servicio</label><input type="text" name="claveprodserv" id="claveprodserv">';
					print '</div>';
				}
			}
	...
	if (event.which != 9 && (event.which < 37 || event.which> 40) && jQuery("#price_ttc").val() != '') {
		...
		if (event.which != 9 && (event.which < 37 || event.which> 40) && jQuery("#price_ttc").val() != '') {
			...
			if (!empty($conf->global->PRODUIT_CUSTOMER_PRICES_BY_QTY)) { ?>
				...
				if (jQuery("#qty").val() < pbqqty) { jQuery("#qty").val(pbqqty); } if (jQuery("#remise_percent").val() < pbqpercent) { jQuery("#remise_percent").val(pbqpercent); } } else { jQuery("#pbq").val(''); } <?php
																																																					}
																																																						?> //Deal with supplier if (jQuery('#idprodfournprice').val()>0)
					{
					...
					if (jQuery("#qty").val() < qty) { jQuery("#qty").val(qty); } if (jQuery("#remise_percent").val() < discount) { jQuery("#remise_percent").val(discount); } <?php
																																												if (!empty($conf->global->PRODUIT_AUTOFILL_DESC) && $conf->global->PRODUIT_AUTOFILL_DESC == 1) {
																																												?> var description=$('option:selected', this).attr('data-description'); if (typeof description=='undefined' ) { description=jQuery('#idprodfournprice').attr('data-description'); } console.log("Load desciption into text area : "+description);
				<?php
																																													if (!empty($conf->global->FCKEDITOR_ENABLE_DETAILS)) { ?>
			if (typeof CKEDITOR == " object" && typeof CKEDITOR.instances !="undefined" ) { var editor=CKEDITOR.instances['dp_desc']; if (editor) { editor.setData(description); } } <?php
																																													} else { ?> jQuery('#dp_desc').text(description); <?php
																																																									}
