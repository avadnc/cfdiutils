 ...
 */
dol_include_once('/custom/cfdiutils/core/modules/cfdiutils/modules_cfdifacture.php');


/**
 *	Class of numbering module Mercure for invoices
 */
class mod_cfdifacture_mercure extends ModeleNumRefCfdiactures
{
	/**
	...
	public function info()
	{
		...
		$texte = $langs->trans('GenericCFDINumRefModelDesc') . "<br>\n";
		$texte .= '<form action="' . $_SERVER["PHP_SELF"] . '" method="POST">';
		$texte .= '<input type="hidden" name="token" value="' . newToken() . '">';
		$texte .= '<input type="hidden" name="action" value="updateMask">';
		$texte .= '<input type="hidden" name="maskconstinvoice" value="FACTURE_CFDIMERCURE_MASK_INVOICE">';
		$texte .= '<input type="hidden" name="maskconstcredit" value="FACTURE_CFDIMERCURE_MASK_CREDIT">';
		$texte .= '<input type="hidden" name="maskconstdeposit" value="FACTURE_CFDIMERCURE_MASK_DEPOSIT">';
		...
		$texte .= '<td class="right">' . $form->textwithpicto('<input type="text" class="flat minwidth175" name="maskinvoice" value="' . $conf->global->FACTURE_CFDIMERCURE_MASK_INVOICE . '">', $tooltip, 1, 1) . '</td>';
		...
		$texte .= '<td class="right">' . $form->textwithpicto('<input type="text" class="flat minwidth175" name="maskcredit" value="' . $conf->global->FACTURE_CFDIMERCURE_MASK_CREDIT . '">', $tooltip, 1, 1) . '</td>';
		$texte .= '</tr>';

		// Prefix setting of deposit
		$texte .= '<tr><td><span class="opacitymedium">' . $langs->trans("Mask") . ' (' . $langs->trans("InvoiceDeposit") . '):</span></td>';
		$texte .= '<td class="right">' . $form->textwithpicto('<input type="text" class="flat minwidth175" name="maskdeposit" value="' . $conf->global->FACTURE_CFDIMERCURE_MASK_DEPOSIT . '">', $tooltip, 1, 1) . '</td>';
		$texte .= '</tr>';
	...
	public function getNextValue($objsoc, $invoice, $mode = 'next')
	{
		...
		if (is_object($invoice) && $invoice->type == 1) {
			...
			$mask = $conf->global->FACTURE_CFDIMERCURE_MASK_INVOICE;
		} elseif (is_object($invoice) && $invoice->type == 2) {
			$mask = $conf->global->FACTURE_CFDIMERCURE_MASK_CREDIT;
		} elseif (is_object($invoice) && $invoice->type == 3) {
			$mask = $conf->global->FACTURE_CFDIMERCURE_MASK_DEPOSIT;
		} else {
			$mask = $conf->global->FACTURE_CFDIMERCURE_MASK_INVOICE;
		}
		if (!$mask) {
