...
/**
 *    \file       htdocs/core/modules/facture/doc/pdf_cfdi.modules.php
 *    \ingroup    facture
 *    \brief      File of class to generate customers invoices from cfdi model
 */
...
dol_include_once('/cfdiutils/vendor/autoload.php');
use Endroid\QrCode\QrCode;
use \CfdiUtils\TimbreFiscalDigital\TfdCadenaDeOrigen;


/**
 *    Class to generate the customer invoice PDF with template cfdi
 */
class pdf_cfdi extends ModelePDFFactures
{
    /**
    ...
    public function __construct($db)
    {
        ...
        $this->db = $db;
        $this->name = "cfdi";
        $this->description = $langs->trans('PDFcfdiDescription');
    ...
    public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
    {
        ...
        $comprobante = $this->_getXmlData($object->id);
    ...
    protected function _tableau_info(&$pdf, $object, $posy, $outputlangs)
    {
        ...
        // \PHPQRCode\QRcode::png($this->_getQrCode($object->id), $conf->facture->dir_output."/".$object->ref."/".$object->ref.".png", 'L', 4, 2);


        $qrCode = new QrCode($this->_getQrCode($object->id));
        ...
        $pdf->SetXY($this->marge_gauche, $posy);
        $pdf->image('@'.$qrCode->writeString(),$posxval - 42, $posy + 2,30,30);

        //No Certificado SAT
        $pdf->SetFont('', 'B', $default_font_size - 2);
        $pdf->SetXY($posxval -12 , $posy + 2);
        $pdf->MultiCell(80, 5, 'No. Certificado SAT:', 0, 'L');

        $pdf->SetFont('', '', $default_font_size - 2);
        $pdf->SetXY($posxval +18, $posy + 2);
        $pdf->MultiCell(80, 5, $tfd['NoCertificadoSAT'], 0, 'L');

        //Fecha Certificacion
        $pdf->SetFont('', 'B', $default_font_size - 2);
        $pdf->SetXY($posxval - 12, $posy + 5);
        $pdf->MultiCell(80, 5, 'Fecha Certificación:', 0, 'L');

        $pdf->SetFont('', '', $default_font_size - 2);
        $pdf->SetXY($posxval + 18, $posy + 5);
        $pdf->MultiCell(80, 5, $tfd['FechaTimbrado'], 0, 'L');


         //Sello CFDI
        $pdf->SetFont('', 'B', $default_font_size - 2);
        $pdf->SetXY($posxval - 12, $posy + 9);
        $pdf->MultiCell(80, 5, 'Sello CFDI:', 0, 'L');

        $pdf->SetFont('', '', $default_font_size - 5);
        $pdf->SetXY($posxval + 4, $posy + 9);
        $pdf->MultiCell(140, 5, $tfd['SelloCFD'], 0, 'L');

        //Sello SAT
        $pdf->SetFont('', 'B', $default_font_size - 2);
        $pdf->SetXY($posxval - 12, $posy + 16);
        $pdf->MultiCell(80, 5, 'Sello SAT:', 0, 'L');

        $pdf->SetFont('', '', $default_font_size - 5);
        $pdf->SetXY($posxval + 4, $posy + 16);
        $pdf->MultiCell(140, 5, $tfd['SelloSAT'], 0, 'L');


        // Cadena Original
        $pdf->SetFont('', 'B', $default_font_size - 3);
        $pdf->SetXY($posxval - 12, $posy + 22);
        $pdf->MultiCell(80, 5, 'Cadena SAT:', 0, 'L');

        $cadenaSat = \CfdiUtils\Nodes\XmlNodeUtils::nodeToXmlString($tfd);
        $tfdXmlString =$cadenaSat;
        $builder = new TfdCadenaDeOrigen();
        $tfdCadenaOrigen = $builder->build($tfdXmlString);

        $pdf->SetFont('', '', $default_font_size - 5);
        $pdf->SetXY($posxval + 4, $posy + 22);
        $pdf->MultiCell(140, 5, $tfdCadenaOrigen, 0, 'L');
    ...
    protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $outputlangsbis = null)
    {
        ...
        if (!empty($conf->global->PDF_USE_ALSO_LANGUAGE_CODE) && is_object($outputlangsbis)) {
            ...
            if ($object->type == 0) {
                if ($this->situationinvoice) {
                    $title .= $outputlangsbis->transnoentities("InvoiceSituation");
                }
        ...
        $posy += 5;
        $pdf->SetFillColor(255, 255, 255);
        $pdf->RoundedRect($posx - 80, $posy + 32.5, 190, 35, 1.5, '1234', 'FD');

        $pdf->SetXY($posx, $posy);
        $pdf->SetTextColor(0, 0, 60);
        $textref = $outputlangs->convToOutputCharset($object->ref);
        ...
        if ($object->ref_client) {
            ...
            $pdf->SetXY($posx, $posy);
            $pdf->SetTextColor(0, 0, 60);
            $pdf->MultiCell($w, 3, "Orden de Compra : " . $outputlangs->convToOutputCharset($object->ref_client), '', 'R');
        }
        ...
        if ($showaddress) {
            ...
            $pdf->MultiCell($widthrecbox + 30, 4, $carac_emetteur, 0, 'L');

            $pdf->SetXY($posx + 40, $posy + 8.6);
            $pdf->SetFont('', 'C', $default_font_size);
            $pdf->MultiCell($widthrecbox - 2, 4, 'RFC: ' . $conf->global->MAIN_INFO_SIREN, 0, 'L');
            ...
            // Show recipient name
            $pdf->SetFont('', 'B', $default_font_size - 1);
            $pdf->SetXY($posx - 90, $posy + 8);
            $pdf->MultiCell($widthrecbox, 4, $outputlangs->transnoentities("Customer") . ':', 0, 'L');

            $pdf->SetXY($posx - 70, $posy + 8);

            if(strlen($carac_client_name) >= 40){
            ...
            $pdf->MultiCell($widthrecbox + 10, 4, $carac_client_name, 0, 'L');

            $pdf->SetXY($posx - 90, $posy + 12);
            $pdf->SetFont('', 'B', $default_font_size);
            $pdf->MultiCell($widthrecbox, 4, $outputlangs->transnoentities("ProfId1MX"), 0, 'L');

            $pdf->SetXY($posx - 70, $posy + 12);
            $pdf->SetFont('', '', $default_font_size);
            $pdf->MultiCell($widthrecbox, 4, $object->thirdparty->idprof1, 0, 'L');
            ...
            $pdf->MultiCell($widthrecbox, 4, "Direccion:", 0, 'L');

             if(strlen($carac_client) >= 40){
            ...
            //Mostrar email
            $pdf->SetFont('', 'B', $default_font_size - 1);
            $pdf->SetXY($posx - 90, $posy + 7.5);
            $pdf->MultiCell($widthrecbox, 4, 'Correo:', 0, 'L');

            $pdf->SetFont('', '', $default_font_size - 1);
            $pdf->SetXY($posx - 70, $posy + 7.5);
            $pdf->MultiCell($widthrecbox, 4, $object->thirdparty->email, 0, 'L');

            //Mostrar Telefono
            $pdf->SetFont('', 'B', $default_font_size - 1);
            $pdf->SetXY($posx - 90, $posy + 12);
            $pdf->MultiCell($widthrecbox, 4, $outputlangs->transnoentities("Phone") . ':', 0, 'L');
            ...
            $usuario->fetch($object->user_author);

            $pdf->SetFont('', 'B', $default_font_size - 1);
            $pdf->SetXY($posx - 90, $posy + 17);
            $pdf->MultiCell($widthrecbox, 4, 'Atendindo Por:', 0, 'L');

            $pdf->SetFont('', '', $default_font_size - 1);
            $pdf->SetXY($posx - 65, $posy + 17);
            $pdf->MultiCell($widthrecbox, 4, $usuario->lastname . ', ' . $usuario->firstname, 0, 'L');

            //USO CFDI
            $pdf->SetFont('', 'B', $default_font_size - 1);
            $pdf->SetXY($posx + 5, $posy - 10);
            $pdf->MultiCell($widthrecbox, 4, "Uso del CFDI:", 0, 'L');

            $pdf->SetFont('', '', $default_font_size - 1);
            $pdf->SetXY($posx + 35, $posy - 10);
            $pdf->MultiCell($widthrecbox, 4, $this->_getUsoCfdi($object->array_options['options_usocfdi']) . ' - ' . $object->array_options['options_usocfdi'], 0, 'L');

            //Forma de Pago CFDI
            $pdf->SetFont('', 'B', $default_font_size - 1);
            $pdf->SetXY($posx + 5, $posy - 5);
            $pdf->MultiCell($widthrecbox, 4, "Forma de Pago:", 0, 'L');

            $pdf->SetFont('', '', $default_font_size - 2);
            $pdf->SetXY($posx + 35, $posy - 5);
            $pdf->MultiCell($widthrecbox, 4, $this->_getFormaPago($object->array_options['options_formpagcfdi']) . ' - ' . $object->array_options['options_formpagcfdi'], 0, 'L');

            //UUID

            $pdf->SetFont('', 'B', $default_font_size - 1);
            $pdf->SetXY($posx + 5, $posy);
            $pdf->MultiCell($widthrecbox, 4, "UUID:", 0, 'L');
            $tfd = $comprobante->searchNode('cfdi:Complemento', 'tfd:TimbreFiscalDigital');

            $pdf->SetFont('', '', $default_font_size - 1);
            $pdf->SetXY($posx + 35, $posy);
            $pdf->MultiCell($widthrecbox, 4, $tfd['UUID'], 0, 'L');

            //CSD
            $pdf->SetFont('', 'B', $default_font_size - 1);
            $pdf->SetXY($posx + 5, $posy + 5);
            $pdf->MultiCell($widthrecbox, 4, "Certificado CSD:", 0, 'L');

            $pdf->SetFont('', '', $default_font_size - 1);
            $pdf->SetXY($posx + 35, $posy + 5);
            $pdf->MultiCell($widthrecbox, 4, $comprobante['NoCertificado'], 0, 'L');

            //Fecha y hora de Emisión
            $pdf->SetFont('', 'B', $default_font_size - 1);
            $pdf->SetXY($posx + 5, $posy + 10);
            $pdf->MultiCell($widthrecbox, 4, "Fecha de Emisión:", 0, 'L');

            $pdf->SetFont('', '', $default_font_size - 1);
            $pdf->SetXY($posx + 35, $posy + 10);
            $pdf->MultiCell($widthrecbox, 4, $comprobante['Fecha'], 0, 'L');
            ...
            $pdf->MultiCell($widthrecbox, 4, $this->_getRegimenFiscal($conf->global->MAIN_INFO_SOCIETE_FORME_JURIDIQUE) . ' - ' . $conf->global->MAIN_INFO_SOCIETE_FORME_JURIDIQUE, 0, 'L');
    ...
    protected function _getxmlData($idFactura)
    {
        ...
        if ($num > 0) {
            $xmlContents = $this->db->fetch_object($result);
        ...
        protected function _getQrCode($idFactura)
    {
    ...
    protected function _getUsoCfdi($usocfdi)
    {
    ...
    private function _getFormaPago($formapago)
    {
        switch ($formapago) {
            case 'PUE':
    ...
    private function _getRegimenFiscal($regimen)
    {
        switch ($regimen) {
            case 605:
