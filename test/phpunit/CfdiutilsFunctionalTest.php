...
/**
 * \file    test/phpunit/CfdiutilsFunctionalTest.php
 * \ingroup cfdiutils
 ...
 *
 * @package Testcfdiutils
 */
class CfdiutilsFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modCfdiutils')->click();
		} else {
			// Disable the module
			$this->byHref('modCfdiutils')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/cfdiutils/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('cfdiutils/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/cfdiutils/admin/about.php');
		$this->authenticate();
		return $this->assertContains('cfdiutils/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('cfdiutilswidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modCfdiutils_CfdiutilsTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modCfdiutils_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
